...
controller.list_historicos = async (req, res) => {
    try{
        ...
        if (log == true) {
            ...
            "ISNULL(CONVERT(varchar,DATEADD(minute,"+req.session.diffhorario+",c.FechaHoraFin),20), CONVERT(varchar,DATEADD(minute,"+req.session.diffhorario+",GETDATE()),20)) as fechafin, c.LastMsgLat, c.LastMsgLong, c.Active, 0 as Locked, ISNULL(t.DistanciaReal,0) as DistanciaCompleta, t.Origen"+
