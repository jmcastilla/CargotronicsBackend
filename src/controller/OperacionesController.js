...
// FUNCION QUE RETORNA EL LISTADO DE CONTRATOS ACTIVOS, GRILLA DE TRAFICO
controller.get_contratostraficooculto = async (req, res) => {
    try{
        var token = req.headers.authorization;
        if (!token) {
            return res.json({ success: false, message: 'Token is missing' });
...
controller.get_reportesdevice2 = async (req, res) => {
    try{
        var token = req.headers.authorization;
        if (!token) {
            return res.json({ success: false, message: 'Token is missing' });
        ...
        if (!token) {
            ...
            jwt.verify(token, 'secret_key', async (err, decoded) => {
                if (err) {
                    ...
                    consulta += " FROM LokDeviceID INNER JOIN LokProyectos ON IdProyecto = LokDeviceId.FKLokProyecto LEFT JOIN LokMcccPais ON cod_pais = FKPais and UltMccc = mcc LEFT JOIN LokContractID ON LokDeviceID.LastContractID = ContractID LEFT JOIN ICEmpresa ON FKICEmpresa = IdEmpresa LEFT JOIN ICRutas ON IdRuta = FKICRutas  INNER JOIN LokDeviceIDEstado ON Estado = IDEstado WHERE FKLokTipoEquipo <> 1 AND FKLokTipoEquipo <> 4 AND FKLokTipoEquipo <> 8 ";
        ...
        if (!token) {
            ...
            jwt.verify(token, 'secret_key', async (err, decoded) => {
                if (err) {
                    ...
                    var consulta= "SELECT r.Id_Reporte, r.NombreReporte, r.Id_PowerBI, r.BitFiltroProyecto, r.BitFiltroEmpresa, c.Id_ReportCategoria, c.ReportCategoria, r.RolesAutorizados, IdBItype FROM LokReportesPBI as r inner join LokReportCategorias as c on c.Id_ReportCategoria = r.Fk_ReportCategoria WHERE r.Fk_LokProyecto="+decoded.proyecto;
                    let resultado=await sqlconfig.query(consulta);
                    res.json({success : true, data : resultado.recordsets[0], rol: decoded.tipouser});
                }
            });
        }
    }catch(err){
                ...
                if (err) {
                    ...
                    if(proyectodestino !== 1 ){
                      let consulta = `
                          UPDATE d
                          SET
                              d.FKLokProyecto = ${proyectodestino},
                              d.EmpresaFija = p.EmpresaPrincipal
                          FROM LokDeviceID d
                          JOIN LokProyectos p ON p.IDProyecto = ${proyectodestino}
                          WHERE d.DeviceID IN (${deviceArray.join(',')})
                      `;
                        res.json({success : await sqlconfig.query(consulta)});
                    }else{
        ...
        if (!token) {
            ...
            jwt.verify(token, 'secret_key', async (err, decoded) => {
                if (err) {
                    ...
                    let consulta = `UPDATE ValitronicsChequeo SET nombre_conductor1 = '${conductor}', placa1 = '${placa}', observaciones1 = CONCAT (ISNULL(observaciones1, '') ,' ${observaciones}')  WHERE Fk_ContractID ='${contrato}'`;
                    console.log(consulta);
                    res.json({success : await sqlconfig.query(consulta)});
        ...
        if (!token) {
            ...
            jwt.verify(token, 'secret_key', async (err, decoded) => {
                if (err) {
                    ...
                    var consulta = "SELECT TOP 100 DeviceID, FKCtOperadorGPS as FkLokCommOp, FKLokProyecto, UsuarioS, ClaveS FROM LokDeviceID where DeviceID LIKE '%"+req.body.Placa+"%' and FKLokTipoEquipo=12 and FKLokProyecto="+decoded.proyecto;
                    let resultado=await sqlconfig.query(consulta);
                    res.json({success : true, data : resultado.recordsets[0]});
                }
            });
...
controller.get_iconomapcenter = async (req, res) => {
    try{
        var token = req.headers.authorization;
        if (!token) {
            res.json({ success: false, message: 'Token is missing' });
            ...
            jwt.verify(token, 'secret_key', async (err, decoded) => {
                if (err) {
                    ...
                    var consulta = "SELECT dbo.IconoMapcenter('"+req.body.Device+"') AS icono";
                    let resultado=await sqlconfig.query(consulta);
                    res.json({success : true, data : resultado.recordsets[0]});
                }
            });
...
controller.existe_devicegps = async (req, res) => {
    try{
        var token = req.headers.authorization;
        if (!token) {
            res.json({ success: false, message: 'Token is missing' });
            ...
            jwt.verify(token, 'secret_key', async (err, decoded) => {
                if (err) {
                    ...
                    var consulta = "SELECT DeviceID FROM LokDeviceID where DeviceID = '"+req.body.Placa+"' and FKLokTipoEquipo=12";
                    let resultado=await sqlconfig.query(consulta);
                    res.json({success : true, data : resultado.recordsets[0]});
                }
            });
