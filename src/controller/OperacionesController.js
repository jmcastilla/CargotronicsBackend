...
controller.get_contratostrafico = async (req, res) => {
    try{
        ...
        if (log == true) {
            ...
            "c.LastReportNota, tr.TipoReporte, c.LastReportUbica+ ' ('+CONVERT(NVARCHAR(20), DATEDIFF(MINUTE, LastReportTime, DATEADD(MINUTE,"+req.session.diffhorario+", GETDATE())))+' min)' as LastReportUbica, "+
            "d.Ciudad+': '+d.Location+ CASE WHEN ContadorGps <> 0 THEN ' ('+CONVERT(NVARCHAR(20), ContadorGps)+')' ELSE '' END as Ciudad, "+
            "DATEADD(MINUTE, DATEDIFF(MINUTE, GETUTCDATE(), GETDATE()) + "+req.session.diffhorario+", LoksysServerTime) as LoksysServerTime, "+
